...
const UserPayments = () => {
  ...
  fechaCreacionUsuario.setMonth(fechaCreacionUsuario.getMonth() - 10); // Mostrar 7 meses hacia atras
  ...
  const handlePayAbono = async (abono) => {
    try {
      // Realizar la petición para obtener la imagen
      const response = await qrApi.get('pago/qr');
      setImageURL(response.data.imageUrl); // Suponiendo que la respuesta contiene la URL de la imagen
            ...
            {isModalOpen && selectedAbono && (
              <div className="modal">
                <div className="modal-content">
                  ...
                  <img src={imageURL} alt="Imagen Aleatoria QR" />
